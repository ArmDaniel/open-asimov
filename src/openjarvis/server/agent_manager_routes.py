...
def _make_lightweight_system(
    ...
    """Build a minimal system with a plain llama.cpp engine.
    ...
    llama.cpp engine directly (no health checks or model discovery that
    could interfere with in-flight local requests).
    """
    try:
        ...
        if cfg is None:
            ...
            cfg = load_config()
        host = cfg.engine.llamacpp.host if cfg else ""
        plain_engine = LlamaCppEngine(host=host) if host else LlamaCppEngine()
